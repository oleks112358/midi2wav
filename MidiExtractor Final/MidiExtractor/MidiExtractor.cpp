...
static void print_help(bool bOnlyUsage)
{
	...
	std::cout << "Usage:midi-convert.exe [input-midi-file-name.mid] [channel-number] [our-sound-font-file.sf2] [bpm]" << std::endl;
	std::cout << "The required arguments:" << std::endl;
	...
	std::cout << "  [BPM]                               ... desired BPM for the file." << std::endl;
}
...
int main(int argc, char* argv[])
{
	...
	if (CMidiFileHelper::ct_is_midifile(clSrcMidiFile) != TRUE)
	{
	...
	if (!clsoundfontFilePath.empty())
	{
		...
		char* cpReducedBuffer = clFileHelper.fnGenerateFileForChannel(iChannelToRetain, iDesiredBPM , iBufferSize);

		wav_filename = clSrcMidiFile.substr(0, clSrcMidiFile.find_last_of('.')) + ".wav";

		// Now lets save this MIDI buffer as a Wav file.
		__gfnSaveMidiBufferAsWavFile(cpReducedBuffer, iBufferSize, wav_filename.c_str(), clsoundfontFilePath.c_str() , iDesiredBPM );

		if (cpReducedBuffer)
			CT_FREE(cpReducedBuffer);
		cpReducedBuffer = nullptr;
	}
